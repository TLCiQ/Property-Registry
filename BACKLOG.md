...
| MAP-02 | DONE | ui | Troubadour 3D orbit still mesh-clipped after MAP-01: per-frame `heading` auto-rotate collapsed camera range into photoreal tiles. Fix in dale-chat `Building3DMap.tsx` (`flyCameraAround` + RELATIVE_TO_GROUND, steady-gated orbit, no extrusion). Deployed `91f962f`; production Ready on tlciq-platform.vercel.app (2026-07-15). | Jul 15 | Jul 15 |
| CAM-01 | DONE | ui | Sent enrichment campaign UI: status banner + tracking-first layout; hide primary Send CTA when active send exists; header **View campaign status** scrolls to monitor; **Resend with changes** secondary only. Deployed `6d466c2` (UI `0a58b1f`); production Ready on https://tlciq-platform.vercel.app (2026-07-15). | Jul 15 | Jul 15 |
| CAM-02 | DONE | bug | Enrich seed hardcoded `MH-REGISTRY-ENRICH-001` → Troubadour send (13:04Z Jul 15) hijacked MH campaign; property UI showed unsent. Fixed property-scoped campaign codes + reuse-by-property_id; repaired TB `REG-ENRICH-095960E35B22` + restored MH. Deployed `6d466c2` (UI `0a58b1f`); production Ready on https://tlciq-platform.vercel.app (2026-07-15). | Jul 15 | Jul 15 |
...
| CSL-01 | DONE | pipeline | CSL fixed/loose FF&E Production sync (`batch-ingest-csl-furniture.mjs`): `26-001`→`26-001-I` (676u/1508 SKU), `26-003`→`26-025-I` (238u/889 SKU), `26-009`→`26-009-I` (+97 SKU on Ann Arbor). `26-008` loose already synced. | Jul 7 | Jul 7 |
| CSL-02 | LATER | pipeline | CSL brand projects without Production deals: East Lansing `26-1093-D`, Bloomington `26-1468-D`/`26-179-I`, Lincoln `27-006-I`/`27-007-I` — need deal creation or alternate BOM source. | Jul 7 | Jul 7 |
| CSL-03 | LATER | pipeline | `sync-production-to-registry.mjs` step 7 overwrites `property_registry.total_units` on multi-deal properties (Ann Arbor loose+fixed). Recalculate from `property_units` count or sum across deals. | Jul 7 | Jul 7 |
| CSL-04 | DONE | pipeline | CSL Sales workbook PH ingest (`ingest-csl-workbook-ph.mjs`): Box Sales …/Project Workbooks/CSL → `source=ph_csl_workbook`; Lincoln II applied via interim Sage invoice PDFs (4 fixed + 15 loose SKUs). Re-run when Sales xlsx uploaded. | Jul 8 | Jul 8 |
