...
| CAM-01 | DONE | ui | Sent enrichment campaign UI: status banner + tracking-first layout; hide primary Send CTA when active send exists; header **View campaign status** scrolls to monitor; **Resend with changes** secondary only. Deploy dale-chat pending. | Jul 15 | Jul 15 |
| CAM-02 | DONE | bug | Enrich seed hardcoded `MH-REGISTRY-ENRICH-001` → Troubadour send (13:04Z Jul 15) hijacked MH campaign; property UI showed unsent. Fixed property-scoped campaign codes + reuse-by-property_id; repaired TB `REG-ENRICH-095960E35B22` + restored MH. **Deploy dale-chat required.** | Jul 15 | Jul 15 |
