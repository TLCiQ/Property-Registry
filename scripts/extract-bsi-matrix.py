...
def main() -> None:
    ...
    sheet_info = find_data_sheet(wb)
    if not sheet_info:
        raise SystemExit("Could not detect matrix sheet/columns")

    sheet_name, header_row, col_idx = sheet_info
    ws = wb[sheet_name]
    ...
    for row in ws.iter_rows(min_row=header_row + 1, values_only=True):
        ...
        if not unit:
            continue
        unit_type = str(row[type_col]).strip() if type_col is not None and type_col < len(row) and row[type_col] else None
        level = str(row[level_col]).strip() if level_col is not None and level_col < len(row) and row[level_col] is not None else None
        area = str(row[area_col]).strip() if area_col is not None and area_col < len(row) and row[area_col] is not None else None
        if unit_type:
    ...
    print(f"Wrote {out_path} — {out['unit_count']} units, {out['unit_type_count']} types")
